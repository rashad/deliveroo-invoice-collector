...
(function() {
    ...
    // Check if email is set, if not show prompt
    function getOrPromptEmail() {
        return new Promise((resolve) => {
            ...
            // Show email prompt
            const email = prompt('📧 Please enter your email address for sending receipts:\n\n(This will be saved in your browser and can be changed later)', '');
    ...
    function addDownloadButton() {
        ...
        if (document.getElementById('deliveroo-tm-button')) {
            return;
        }

        const button = document.createElement('button');
        button.id = 'deliveroo-tm-button';
        button.innerHTML = '📥 Download Receipts';
        ...
        button.onclick = async function() {
            ...
            const userEmail = await getOrPromptEmail();
            if (!userEmail) {
                return;
            }
            
            // Start the download script
            await startDownloadScript(userEmail);
        };
    ...
    async function startDownloadScript(initialEmail) {
        ...
        if (downloadedFiles.length > 0) {
            ...
            emailBtn.onclick = function() {
                const emailTo = userEmail; // Use the stored email
